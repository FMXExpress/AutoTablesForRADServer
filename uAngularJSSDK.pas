...
uses
  ...
  Data.DB, FireDAC.Comp.DataSet, FireDAC.Comp.Client, System.IOUtils, StrUtils;

type
  TAngularJSSDKFrame = class(TFrame)
...
function TAngularJSSDKFrame.GetQueryStringList(ADataSet: TFDDataSet): String;
begin
  Result := MainForm.GetQueryStringList(ADataSet);
end;

procedure TAngularJSSDKFrame.Initialize(AHost,APort: String; ADataSet: TFDDataSet; AInfoDataSet: TFDDataSet);
begin
    Host := AHost;
    Port := APort;
...
function TAngularJSSDKFrame.GenerateSDK(AFileName: String): String;
var
  I: Integer;
  SL: TStringList;
...
begin
      ...
      begin
        ...
        for I := 0 to SL.Count-1 do
          begin
            if SL[I]='format' then
              begin
               QueryString := QueryString + '''+(aformat!='''' ? '''+ IfThen(I=0,'?','&') + SL[I] + '=''+a' + SL[I] + IfThen(I=SL.Count-1,'','+''') + ' : '''')';
              end
             else
              QueryString := QueryString + IfThen(I=0,'?','&') + SL[I] + '=''+a' + SL[I] + IfThen(I=SL.Count-1,'','+''');
            ParamString := ParamString + IfThen(I=0,'',', ') + 'a' + SL[I] + IfThen(SL[I]='format',' = ''''','');
          end;
        if EndPointTable.FieldByName('RequestType').AsString='POST' then
          begin
            ParamString := '('+ParamString+IfThen(ParamString<>'',',','')+'abody'+')';
          end
        else
          begin
  ...
  SL := TStringList.Create;
  if TFile.Exists(TPath.Combine(ATemplatePath,AngularJSSDKTemplateFile)) then
    SL.LoadFromFile(TPath.Combine(ATemplatePath,AngularJSSDKTemplateFile))
  else
